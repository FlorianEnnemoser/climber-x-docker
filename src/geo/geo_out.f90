...
module geo_out
  ...
  type ts_out
     real(wp) :: sea_level, Aocn, Aveg, Aice, Alake, A_bering, A_shelf
  end type
...
=======
    real(dp) :: lat_coord(1) = [1.0d0]
    real(dp) :: lon_coord(1) = [1.0d0]
>>>>>>> df778f0f08349e72b3a84e5a4d78c98f109289be
  ...
  subroutine geo_diag(geo)
    ...
    print '(a7,i9,7F7.1)','geo',year_now,geo%sea_level,geo%ocn_area_tot*fac,geo%veg_area_tot*fac,geo%ice_area_tot*fac,geo%lake_area_tot*fac, & 
      geo%A_bering*1.e-6,geo%A_shelf*fac
    ...
    if(.not.firstcall) then
      ...
      ann_ts(y)%A_shelf = geo%A_shelf*fac
      ...
      if (time_out_ts_geo) then
        ...
        call nc_write(fnm,"A_shelf", ann_ts(1:y)%A_shelf, dim1=dim_time, start=[n1out], &
    ...
    if (time_out_geo .or. firstcall) then
      ...
      call nc_write(fnm,"z_ocn", geo%z_ocn,dims=[dim_lon,dim_lat,dim_time],start=[1,1,nout],count=[geo%grid%G%nx,geo%grid%G%ny,1],long_name="mean elevation of ocean",units="m",ncid=ncid)
      ...
      call nc_write(fnm,"mask_coast2", geo%mask_coast2,dims=[dim_lon,dim_lat,dim_time],start=[1,1,nout],count=[geo%grid%G%nx,geo%grid%G%ny,1],long_name="coast mask",units="/",ncid=ncid)
